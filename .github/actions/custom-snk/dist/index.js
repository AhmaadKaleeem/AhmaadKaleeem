...
var getGithubUserContribution = async (userName, o) => {
  ...
  if (errors?.[0])
    throw errors[0];
  const cells = data.user.contributionsCollection.contributionCalendar.weeks.flatMap(({ contributionDays }, x) => contributionDays.map((d) => ({
    x,
    y: d.weekday,
    date: d.date,
  ...
  return cells.map((c) => c.x >= maxX - 1 ? { ...c, count: 0, level: 0 } : c);
};
